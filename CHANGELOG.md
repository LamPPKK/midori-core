...
- Added one shared 4,096-byte page-title policy across Linux WebKit UI,
...
- Added a separate weekly official-Maven baseline for every direct Android UI,
  platform and test dependency not already owned by an engine/toolchain gate.
  It verifies every Gradle occurrence, optional Room/Work/KSP families and
  strict checksum entry where enabled.
...
- Added a Windows native site-password library over the Logins CRUD C ABI. It
  lists, adds, updates and confirm-deletes by exact native ID for only the
  selected regular HTTPS origin, revalidates every async boundary and dismisses
  stale credential UI on navigation, tab changes, backgrounding or vault lock.
...
- Re-pinned the WinCairo source preview from an unversioned WebKit `main`
...
- Added a redirect-aware WinCairo navigation-action policy that bounds and
  ...
  token to Windows.
- Added bounded WinCairo WebProcess recovery: one back-forward restore for a
  committed item without an HTTP body, no automatic form resubmission,
  provisional-navigation race protection and a hard stop for repeat
  termination.
