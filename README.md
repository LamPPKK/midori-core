...
Generated files are placed under `app/build/outputs/`. A native-backed release
first runs `scripts/build-adblock-android.sh`, then supplies its output through
`-PxanhAdblockNativeDir` and the pinned NDK through `-PxanhAndroidNdkDir`.
`bundleRelease` creates an **unsigned verification AAB**; CI performs that
native build and gate. A production bundle additionally requires the dedicated
...
Xanh uses the pinned MPL-2.0 `adblock-rust` 0.13.3 engine instead of embedding
the full GPLv3 uBlock Origin WebExtension runtime. The shared
`xanh-adblock-core` supports concurrent request matching through Rust and C
APIs and converts compatible rules to WebKit content-blocker JSON. It ships a
small Xanh-owned offline baseline; third-party lists such as EasyList are not
silently bundled and require their own provenance, attribution and update
policy.

Linux bundles the matching shared core, Apple bundles rule JSON generated by
that exact core, Windows Release artifacts bundle an architecture-matched DLL,
and both the full Android companion app and Android Lite System WebView edition
package the same C ABI for three Android ABIs through JNA. The separate WPE
preview is not covered because published WPEView 0.3.3 exposes no safe request
interception or content-filter host API. Missing or rejected native code
retains only the small audited baseline where a host fallback is available; no
UI claims uBO parity or protection in the unsupported WPE preview.

The first host integration is intentionally narrower than uBO: network rules,
exceptions and convertible cosmetic filters are supported, while scriptlets,
procedural cosmetics, redirect resources, CSP and `removeparam` remain outside
the release claim. Some browser callbacks also cannot safely cover documents,
workers, every redirect or WebSocket. See [`docs/ADBLOCK.md`](docs/ADBLOCK.md) for the engine
choice, platform boundaries, privacy model and release gate.
...
| `xanh-adblock-core/` | Pinned native adblock-rust engine and stable C ABI |
