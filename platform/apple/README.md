...
Content blocking is enabled by default for regular and private tabs. Both app
targets bundle `App/xanh-adblock-baseline.json`, generated byte-for-byte by the
pinned `xanh-adblock-core`/`adblock-rust` WebKit converter. A new or restored
tab waits for `WKContentRuleList` installation before its first load;
compilation failure explicitly falls open. macOS may also use an exact-version
native compiler as a recovery path, while iOS/iPadOS never depend on loading an
unsigned dynamic library. The baseline is intentionally smaller than uBlock
Origin and does not claim scriptlet, procedural-cosmetic or dynamic-firewall
semantics.
