...
case "$edition" in
  source)
    test -x scripts/verify_adblock_latest.py
    test -f scripts/tests/test_verify_adblock_latest.py
    test -f .github/workflows/adblock-core.yml
    test -f xanh-adblock-core/ADBLOCK_RUST.lock
    test -f xanh-adblock-core/Cargo.lock
    test -f xanh-adblock-core/include/xanh_adblock.h
    test -f xanh-adblock-core/filters/xanh-baseline.txt
    test -f xanh-adblock-core/LICENSE
    test -f xanh-adblock-core/tests/c_abi_smoke.c
    test -x scripts/build-adblock-android.sh
    test -f docs/ADBLOCK.md
    test -f platform/apple/App/xanh-adblock-baseline.json
    grep -F 'python3 -B scripts/verify_adblock_latest.py' \
      .github/workflows/adblock-core.yml >/dev/null
    test "$(grep -Fc '      - xanh-adblock-core/**' \
      .github/workflows/adblock-core.yml)" -eq 2
    grep -F 'version=0.13.3' xanh-adblock-core/ADBLOCK_RUST.lock >/dev/null
    grep -F 'revision=886d45dcf5283ce8eddc6d961e7dd27966ab23f2' \
      xanh-adblock-core/ADBLOCK_RUST.lock >/dev/null
    grep -F 'default-features = false' xanh-adblock-core/Cargo.toml >/dev/null
    if grep -F '"single-thread"' xanh-adblock-core/Cargo.toml >/dev/null; then
      echo 'adblock-rust single-thread feature cannot serve concurrent browser hosts' >&2
      exit 1
    fi
    grep -F 'xanh_adblock_engine_should_block' \
      xanh-adblock-core/include/xanh_adblock.h >/dev/null
    grep -F 'xanh_adblock_compile_webkit_json' \
      xanh-adblock-core/include/xanh_adblock.h >/dev/null
    grep -F 'xanh_adblock_engine_create_default' \
      xanh-adblock-core/include/xanh_adblock.h >/dev/null
    grep -F '#define XANH_ADBLOCK_CORE_ABI_VERSION "1.0.0-alpha.1"' \
      xanh-adblock-core/include/xanh_adblock.h >/dev/null
    grep -F 'Compile and run the stable C ABI smoke test' \
      .github/workflows/adblock-core.yml >/dev/null
    grep -F 'Verify the Apple bundled rules are generated by the pinned core' \
      .github/workflows/adblock-core.yml >/dev/null
    test "$(grep -Fc '      - platform/apple/App/xanh-adblock-baseline.json' \
      .github/workflows/adblock-core.yml)" -eq 2
    test "$(grep -Fc 'xanh-adblock-baseline.json in Resources' \
      platform/apple/XanhBrowserApple.xcodeproj/project.pbxproj)" -eq 4
    test "$(grep -Fc '      - path: ../../THIRD_PARTY_NOTICES.md' \
      platform/apple/project.yml)" -eq 2
    test "$(grep -Fc '      - path: ../../xanh-adblock-core/LICENSE' \
      platform/apple/project.yml)" -eq 2
    test "$(grep -Fc 'THIRD_PARTY_NOTICES.md in Resources' \
      platform/apple/XanhBrowserApple.xcodeproj/project.pbxproj)" -eq 4
    test "$(grep -Fc 'LICENSE in Resources' \
      platform/apple/XanhBrowserApple.xcodeproj/project.pbxproj)" -eq 4
    grep -F 'adblockInstallationPending' platform/apple/App/BrowserModel.swift >/dev/null
    grep -F 'isNavigationTemporarilyBlocked' platform/apple/App/BrowserModel.swift >/dev/null
    grep -F 'Content blocking unavailable' platform/apple/App/BrowserView.swift >/dev/null
    grep -F 'CoreWebView2WebResourceRequestSourceKinds.Document' \
      platform/windows/src/XanhBrowser.Windows/BrowserTab.xaml.cs >/dev/null
    if grep -F 'CoreWebView2WebResourceRequestSourceKinds.All' \
      platform/windows/src/XanhBrowser.Windows/BrowserTab.xaml.cs >/dev/null; then
      echo 'WinUI adblock must not register per-tab shared/service-worker interception' >&2
      exit 1
    fi
    grep -F 'RequireReleaseXanhAdblockPackage' \
      platform/windows/src/XanhBrowser.Windows/XanhBrowser.Windows.csproj >/dev/null
    grep -F -- '-p:XanhAdblockNativeDll=' \
      .github/workflows/windows.yml >/dev/null
    grep -F 'BuiltNativeArtifactLoadsAndBlocksBaselineWhenProvided' \
      .github/workflows/windows.yml >/dev/null
    grep -F 'Microsoft Visual Studio/Installer/vswhere.exe' \
      .github/workflows/windows.yml >/dev/null
    grep -F "install(FILES \"\${XANH_ADBLOCK_CORE_LIBRARY_REAL}\"" \
      CMakeLists.txt >/dev/null
    grep -F -- '-DXANH_ENABLE_ADBLOCK_RUST=ON' \
      flatpak/io.github.lamppkk.xanhbrowser.yml >/dev/null
    grep -F 'flatpak/native/libxanh_adblock_core.so' \
      flatpak/io.github.lamppkk.xanhbrowser.yml >/dev/null
    grep -F 'Build the pinned native blocker for the Flatpak sandbox' \
      .github/workflows/flatpak.yml >/dev/null
    grep -F 'Verify the installed application resolves the bundled blocker' \
      .github/workflows/desktop.yml >/dev/null
    grep -F 'cargo +1.97.1 build --locked --release' \
      .github/workflows/desktop.yml >/dev/null
    grep -F 'win.toggle-adblock' desktop/browser-window.vala >/dev/null
    grep -F 'adblock-enabled' plugins2/adblock.vala >/dev/null
    grep -F "connect.facebook.net^\$third-party" plugins2/adblock.vala >/dev/null
    grep -F 'queue_tab_navigation' desktop/browser-window.vala >/dev/null
    grep -F 'adblock_navigation_ready (TabRecord? record = null)' \
      desktop/browser-window.vala >/dev/null
    grep -F 'uri != "about:blank" && !adblock_navigation_ready (tab)' \
      desktop/browser-window.vala >/dev/null
    grep -F 'application.get_windows ()' desktop/browser-window.vala >/dev/null
    grep -F 'error is AdblockHostError.BUSY' desktop/browser-window.vala >/dev/null
    grep -F 'MAX_FILTER_LINES = 500000' desktop/adblock-source.vala >/dev/null
    grep -F 'missing_rules.contains (rule)' desktop/adblock-source.vala >/dev/null
    grep -F 'net.java.dev.jna:jna:5.18.1@aar' app/build.gradle >/dev/null
    grep -F ':app:verifyAdblockNativeRelease' \
      .github/workflows/android-lite.yml >/dev/null
    grep -F -- '-PxanhAdblockNativeDir=' \
      .github/workflows/android-lite.yml >/dev/null
    grep -F "ndk;29.0.14206865" .github/workflows/android-lite.yml >/dev/null
    grep -F 'Android Lite System WebView' docs/ADBLOCK.md >/dev/null
    grep -F 'Android Lite WPE preview' docs/ADBLOCK.md >/dev/null
    grep -F 'Not integrated.' docs/ADBLOCK.md >/dev/null
    grep -F 'Java Native Access (JNA) 5.18.1' THIRD_PARTY_NOTICES.md >/dev/null
    grep -F 'Brave adblock-rust 0.13.3' THIRD_PARTY_NOTICES.md >/dev/null
    python3 -B -m unittest scripts/tests/test_verify_adblock_latest.py >/dev/null
    ...
    sed -n '/void maybe_recover_tab/,/void show_process_stopped/p' \
      desktop/browser-window.vala | grep -F 'queue_tab_navigation (tab, uri);' >/dev/null
