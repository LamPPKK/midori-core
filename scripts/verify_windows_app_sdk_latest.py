...
OFFICIAL_INDEX_URL = (
    ...
    "microsoft.windowsappsdk/index.json"
)
...
def latest_stable_release(contents: bytes) -> StableRelease:
    ...
    for value in versions:
        ...
        stable[parts] = value
    if not stable:
        raise VerificationError("NuGet index contains no stable Windows App SDK release")
    version = max(stable)
...
def _version_for_package_node(path: Path, node: ET.Element) -> str | None:
    ...
    if attribute is not None and children:
        raise VerificationError(f"{path} declares Windows App SDK Version twice")
    if len(children) > 1:
        raise VerificationError(
            f"{path} declares multiple Windows App SDK Version elements"
        )
...
def project_windows_app_sdk_pins(root: Path) -> list[tuple[Path, str]]:
    ...
    for path in _project_files(root):
        ...
        for node in project.iter():
            ...
            if STABLE_VERSION_PATTERN.fullmatch(version) is None:
                raise VerificationError(
                    f"{path} uses a non-stable or dynamic Windows App SDK pin: "
                    f"{version or '<empty>'}"
                )
...
def verify_project(root: Path, contents: bytes) -> StableRelease:
    ...
    stale = [
        (path, version)
        for path, version in project_windows_app_sdk_pins(root)
        if version != latest.text
    ]
    if stale:
        detail = ", ".join(f"{path}={version}" for path, version in stale)
        raise VerificationError(
            f"Windows App SDK pins are stale ({detail}); latest stable is {latest.text}"
        )
    return latest
...
def fetch_official_index(url: str) -> bytes:
    ...
    if parsed.scheme != "https" or not parsed.netloc or parsed.username or parsed.password:
        raise VerificationError("Windows App SDK index URL must be credential-free HTTPS")
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "XanhBrowser-WindowsAppSDK-Verifier/1.0"},
    )
    try:
        with urllib.request.urlopen(request, timeout=60) as response:
            ...
            if (
                ...
                raise VerificationError(
                    "Windows App SDK index redirected outside official HTTPS"
                )
        ...
        raise VerificationError(
            f"cannot query official Windows App SDK index: {error}"
        ) from error
    if len(contents) > MAX_INDEX_BYTES:
        raise VerificationError("NuGet index exceeds the 1 MiB safety limit")
    return contents
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fail when Xanh does not use the latest stable Windows App SDK."
    )
    parser.add_argument(
        "--root",
        type=Path,
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"Windows App SDK latest-stable verification failed: {error}",
            file=sys.stderr,
        )
        return 1
    print(f"Verified latest stable Microsoft.WindowsAppSDK {release.text}")
    return 0
